...
if [ -f "$MODEL_FILE" ]; then
    ...
    echo "[OK] MegaLoc model baked in: $MODEL_FILE ($MODEL_SIZE)"
else
    echo "[ERROR] MegaLoc model NOT found at $MODEL_FILE! Rebuild Docker image."
fi
if [ -d "$HUB_CACHE" ]; then
    echo "[OK] MegaLoc architecture cached at $HUB_CACHE"
else
    echo "[ERROR] MegaLoc architecture NOT cached! Rebuild Docker image."
fi
